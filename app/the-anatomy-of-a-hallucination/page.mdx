...
<div className="flex justify-center mx-auto">
  <Image 
    src={Vocab} 
    alt="Vocabulary of Mistral-7b-v0.1"
    />
</div>
<FigCaption>
...
<div className="my-12">
<div className="flex justify-center mx-auto -mx-[calc(32vw-50%)]">
    <Image 
      src={Image4}
      className="w-screen"
    />
...
<FigCaption>
  Note: The red bar isn't the actual loss, but it is proportional.
</FigCaption>
</div>
...
I could generate coherent stories in elementary school by filling in the blanks of a Mad Lib template. LLMs do something similar, they just do so by noticing what tokens typically come next based on similar contexts from its training data.
...
Instruction Fine-Tuning (IFT) happens after pretraining. **The goal here is to take the weights learned during pretraining and fine-tune (update) them so that the LLM can "behave" like an assistant.** This involves teaching it how to provide helpful answers in response to instructions <FootnoteRef id="1" />.
...
**Here's the key: if the instruction-tuning dataset only includes examples that follow these biographical templates, it will apply it to all such prompts, even when Person X doesn't exist <FootnoteRef id="1" />.** For example, here's what it generates about Blake Bourne:
