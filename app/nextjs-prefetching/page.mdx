...
The server responds with the payload that was prerendered and cached during build. **This RSC payload contains everything the client needs to render the `/about` page.** Note how the body of the `/about` page component is directly encoded in the payload: 
...
As mentioned before, responses for dynamic routes are not prerendered as part of the build process. This makes prefetching for dynamic routes a bit more involved.
...
But since this is a dynamic route, the server has no cached payload to return. Instead of running code to generate the payload, the server opts to return what is essentially an empty response. **In other words, prefetching is skipped by default** for dynamic routes<FootnoteReference id="2" />.
