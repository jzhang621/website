...
<h1 className="text-4xl font-bold">Embeddings Unlock New Functionality</h1>
<div className="mt-2 text-lg text-gray-500">{metadata.description}</div>

In 2018, I tried to programatically visualize what good writing looks like. The goal was to write a program that could take in a piece of writing, and return some dimensions of that writing that could be used to discern its quality.
...
An embedding is a <span className="italic">numerical representation</span> of text. Instead of representing text as a sequence of characters (a string), embeddings represent text as a sequence of numbers. These numbers encode a lot of information, but before we get into what that information is, I want to first demonstrate the importance of numerical representations by looking at how computers represent color.
...
<div className="flex justify-center w-full h-auto mx-auto w-1/2 my-8">
  <Image src={Red} 
...
{/* **The ease with which we can express this transformation computationally is what makes the RGB color model a “good” representation of color**. Here are a couple of other transformations available to us: */}
...
But the effects are even more apparent when we apply the transformation to an entire image:
...
<div className="flex justify-center w-full h-auto mx-auto w-3/5 my-8">
  <Image src={Gray} 
...
The RGB color model is derived from the [Trichromatic Theory of Color Vision](https://www.simplypsychology.org/what-is-the-trichromatic-theory-of-color-vision.html)
", which states that "every combination of color perceived by the brain is a combination of blue, green, and red."<sup>2</sup>. In other words, the RGB color model is rooted in reality - it's a "mathematical understanding" of "how humans perceive color".


a "mathematical model of how we perceive color"
...
These transformations are why representations are important.
