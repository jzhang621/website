...
<MermaidSequenceDiagram 
  diagram={`sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        Client->>Server: navigate to static route
...
With ISR, we can update static routes incrementally, without a full rebuild. Each route can define a **revalidation interval**<FootnoteReference id="1"/> that determines how long its cached response is valid. Requests that fall within the revalidation interval are served from the cache. **But the first request that falls outside the interval triggers a background job to refresh the cache** for the next request.
